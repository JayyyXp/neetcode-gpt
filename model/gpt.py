...
class GPT(nn.Module):
    ...
    def forward(self, context: TensorType[int]) -> TensorType[float]:
        ...
        B, T    = context.shape                                   # batch size, sequence length
        tok_emb = self.token_embedding(context)                   # (B, T, d_model)
        pos     = torch.arange(T, device=context.device)          # [0, 1, 2, ..., T-1]
        pos_emb = self.position_embedding(pos)                    # (T, d_model)
        x       = tok_emb + pos_emb 
        
        # 2. Pass through transformer blocks
        x_hat = self.blocks(x)
        x_hat = self.ln_f(x_hat)
        
        # 3. Apply final LayerNorm, then project to vocab_size
        x_hat = self.lm_head(x_hat)

        # 4. Return logits rounded to 4 decimal places (no softmax)
        return torch.round(x_hat, decimals=4)
